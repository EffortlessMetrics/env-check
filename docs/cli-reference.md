...
> **Note:** CLI flags override configuration file options. See [Configuration Reference](configuration.md#cli-flag-precedence) for precedence rules. Config-only options like `hash_manifests`, `ignore_tools`, `force_required`, and `sources.enabled/disabled` must be set in `env-check.toml`.
