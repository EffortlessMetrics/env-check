...
- `lib.rs`: Core discovery engine plus parsers for `.tool-versions`, `.mise.toml`, and `rust-toolchain*`
- `env-check-sources-node`: Node.js version sources (`.node-version`, `.nvmrc`, `package.json`)
- `env-check-sources-python`: Python version sources (`.python-version`, `pyproject.toml`)
- `env-check-sources-go`: Go module support (`go.mod`)
- `env-check-sources-hash`: Hash manifest support (`scripts/tools.sha256`)
...
cargo fuzz run parse_hash_manifest
